...
export default function ResultsTab({ results, formData }: ResultsTabProps) {
  ...
  const [selectedStrategy, setSelectedStrategy] = useState<"min_cutter_changes" | "min_wastage">(
    "min_wastage",
  )
// console.log(results)
            ...
            <table>
              <thead>
                <tr>
                  <th>Pattern</th>
                  <th>Sets</th>
                  ${Array.from({ length: responseData.no_of_cut }, (_, i) => `<th>Size ${i + 1}</th>`).join('')}
                  <th>Waste/Set (mm)</th>
              ...
              <tbody>
                ${currentResult.cutting_plans.map((plan: any) => {
                  ...
                  return `
                    <tr>
                      ...
                      ${Array.from({ length: responseData.no_of_cut }, (_, i) => {
                        ...
                        return `<td>${size ? `${size.actual_size}${size.uom}` : '-'}</td>`
                      }).join('')}
  ...
  const handleExportExcel = () => {
    ...
    const headers = [
      "Customer Name", "SO No", "Decal Size (mm)", "Max Cuts",
      "Plan Number", "Sets", "Item Name", 
      ...sizeHeaders,
      "Wastage (mm)", "Usage (%)"
    ]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="bg-white border-gray-200 shadow-sm hover:shadow-md transition-all duration-300">
          ...
          <CardContent className="p-6 flex items-center justify-center">
            <PieChart
              data={{
                utilized: currentResult.cutting_plans.reduce((acc: number, plan: any) => acc + plan.usage_percent, 0) / currentResult.cutting_plans.length,
                wastage: 100 - (currentResult.cutting_plans.reduce((acc: number, plan: any) => acc + plan.usage_percent, 0) / currentResult.cutting_plans.length),
              }}
            />
          </CardContent>
        </Card>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            <Card className="bg-white border-gray-200 shadow-sm hover:shadow-md transition-all duration-300">
              <CardContent className="p-3">
                <div className="text-center">
                  <p className="text-xl font-bold text-gray-900 mb-1">{currentStrategy.total_rolls_used}</p>
                  <p className="text-xs font-medium text-gray-600 mb-1">Total Rolls Used</p>
              ...
              <CardContent className="p-3">
                <div className="text-center">
                  <p className="text-xl font-bold text-green-600 mb-1">
                    {(currentResult.cutting_plans.reduce((acc: number, plan: any) => acc + plan.usage_percent, 0) / currentResult.cutting_plans.length).toFixed(2)}%
                  </p>
            ...
            <Card className="bg-white border-gray-200 shadow-sm hover:shadow-md transition-all duration-300">
              <CardContent className="p-3">
                <div className="text-center">
                  <p className="text-xl font-bold text-orange-600 mb-1">{currentStrategy.total_wastage_mm}mm</p>
                  <p className="text-xs font-medium text-gray-600 mb-1">Total Wastage</p>
            ...
            <Card className="bg-white border-gray-200 shadow-sm hover:shadow-md transition-all duration-300">
              <CardContent className="p-3">
                <div className="text-center">
                  ...
                  <p className="text-xl font-bold text-blue-600 mb-1">{currentStrategy.total_cutter_changes}</p>
                  <p className="text-xs font-medium text-gray-600 mb-1">Cutter Changes</p>
                </div>
              </CardContent>
            </Card>
          </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      ...
                      <p className="text-gray-900 font-medium">{responseData.strategies.min_cutter_changes.total_rolls_used}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Wastage (mm):</p>
                      <p className="text-gray-900 font-medium">{responseData.strategies.min_cutter_changes.total_wastage_mm}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Wastage (qty):</p>
                      <p className="text-gray-900 font-medium">{responseData.strategies.min_cutter_changes.total_wastage_qty}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Cutter Changes:</p>
                      <p className="text-gray-900 font-medium">{responseData.strategies.min_cutter_changes.total_cutter_changes}</p>
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <p className="text-gray-600">Rolls Used:</p>
                      <p className="text-gray-900 font-medium">{responseData.strategies.min_wastage.total_rolls_used}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Wastage (mm):</p>
                      <p className="text-gray-900 font-medium">{responseData.strategies.min_wastage.total_wastage_mm}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Wastage (qty):</p>
                      <p className="text-gray-900 font-medium">{responseData.strategies.min_wastage.total_wastage_qty}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Cutter Changes:</p>
                      <p className="text-gray-900 font-medium">{responseData.strategies.min_wastage.total_cutter_changes}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {currentResult.cutting_plans.map((plan: any, index: number) => (
              ...
              >
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 space-y-1 sm:space-y-0">
                  <h4 className="font-semibold text-gray-900">Plan {plan.plan_number} - {plan.item_details.item_name}</h4>
                ...
                <div className="mb-3">
                  <div className="flex rounded-lg overflow-hidden h-8 bg-gray-200">
                    {plan.sizes.filter((size: any) => size.actual_size > 0).map((size: any, sizeIndex: number) => {
                      const totalUsage = plan.total_usage_mm
                      const sizeUsage = size.size_mm
                      const percentage = totalUsage > 0 ? (sizeUsage / (totalUsage + plan.wastage_mm)) * 100 : 0
                      return (
                        <div
                          key={sizeIndex}
                    ...
                    {plan.wastage_mm > 0 && (
                      <div
                        ...
                        style={{
                          width: `${(plan.wastage_mm / (plan.total_usage_mm + plan.wastage_mm)) * 100}%`,
                          backgroundColor: "#E5E7EB",
                        }}
                        title={`Wastage: ${plan.wastage_mm}mm`}
                      >
                        {(plan.wastage_mm / (plan.total_usage_mm + plan.wastage_mm)) * 100 > 5 && "Waste"}
                      </div>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-8 gap-2">
                  {plan.sizes.filter((size: any) => size.actual_size > 0).map((size: any, sizeIndex: number) => (
                    ...
                    >
                      ...
                      <p className="text-gray-900 font-medium text-xs">{size.actual_size}{size.uom}</p>
                      <p className="text-gray-600 text-xs">{size.size_mm}mm</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <th className="text-left p-3 font-medium text-gray-900">Pattern</th>
                  ...
                  <th className="text-left p-3 font-medium text-gray-900">Waste Qty/Set</th>
                  <th className="text-left p-3 font-medium text-gray-900">Total Waste Qty</th>
                  ...
                  <th className="text-left p-3 font-medium text-gray-900">Usage Qty/Set</th>
                  <th className="text-left p-3 font-medium text-gray-900">Total Usage Qty</th>
                ...
                {currentResult.cutting_plans.map((plan: any, index: number) => {
                  ...
                  return (
                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="p-3 text-blue-600 font-medium">Pattern {plan.plan_number}</td>
                      ...
                      <td className="p-3 text-gray-900">{plan.total_wastage_qty}</td>
                      ...
                      <td className="p-3 text-gray-900">{plan.total_usage_qty}</td>
